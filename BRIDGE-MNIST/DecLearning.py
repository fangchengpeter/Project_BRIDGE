...
class DecLearning:
    ...
    '''
    def __init__(self, dataset='MNIST', nodes=50, byzantine=0,
                 total_samples=2000):
    ...
    def Byzantine(self, target, strategy='random', interval=(-1, 0)):
        ...
        if strategy == 'random':
            ...
            min_val = interval[0]
            fal = np.random.random(target.shape) * dist + min_val
        return fal

    def initialization(self):
        pass  # Not needed in TF2 eager mode
    ...
    def Bulyan(self, wb, b):
        ...
        # Part 2 of Bulyan screening for b vector
        ave = []
        grad_dim = len(S_b[0])
        for i in range(grad_dim):
            m_i = [bv[i] for bv in S_b]
            m_i = np.sort(m_i, axis=0)
            m_i = m_i[b:-b]
            m_i = np.mean(m_i, axis=0)
            ave.append(m_i)
    ...
    def communication(self, W, neighbor, b=0,
        ...
        for node, w, bv in zip(W, ave_w, ave_b):
            node.assign([w, bv])

    # Used for ByRDiE
    def communication_w(self, W, neighbor, p, b=0, screen=False, goByzantine=False):
        '''
        ...
        for node, ww, bb in zip(W, _w, _b):
            node.assign([ww.reshape([784, 10]), bb])

    def communication_b(self, W, neighbor, p, b=0, screen=False, goByzantine=False):
        '''
    ...
    def node_update(self, W, data, stepsize=1e-3):
        '''
    ...
    def node_update_w(self, W, data, p, stepsize=1e-4):
        '''
        ...
        '''
        for model, sample, label in zip(W, data.dist_data, data.dist_label):
            g = model.get_gradient_w(
                np.array(sample, dtype=np.float32),
                np.array(label, dtype=np.float32)
            )
            new_g = np.zeros([7840, 1])
            new_g[p] = g.reshape([7840, 1])[p]
            model.apply_gradient_w(new_g.reshape([784, 10]), stepsize)

    def node_update_b(self, W, data, p, stepsize=1e-4):
        '''
